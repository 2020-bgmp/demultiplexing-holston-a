...
def rev_comp(sequence):
    ...
    RevDict = {'A':'T', 'G':'C', 'C':'G', 'T':'A', 'N':'N'}                 #Make our dictionary of canonical base pairs.
    revseq = sequence[::-1]                                                 #Reverse our sequence.
    revComplement = ''
    for base in revseq:
        matchbase = RevDict[base]
        revComplement = revComplement + matchbase
    return revComplement

def rev_comp_maker(indexlist):
    """Given an index list, return a dictionary where the key is the index, and the value is the reverse complement of said index"""
    rev_comp_dict = {}
...
def open_write_files(indexlist):
    ...
    for i in range(len(indexlist)):                                              #Here, we are getting the names of our files.
        ...
        filehandle = open(filename,"w")                                    #'a' option is "append", so it creates files if they are not yet created, and opens them for appending otherwise.
...
def main_demultiplexing(read1,read2,index1,index2,indexfile):
    ...
    with gzip.open(read1, "rt") as r1f, gzip.open(index1, "rt") as i1f, gzip.open(index2, "rt") as i2f, gzip.open(read2, "rt") as r2f:
        ...
        for r1fLine, i1fLine, i2fLine, r2fLine in zip(r1f, i1f, i2f, r2f):
            ...
            if LN % 4 == 0:                                                     #So we have a full record
                ...
                i1qual = RecordHolder[1][3]
                i2qual = RecordHolder[2][3]
                if i1 in indexlist and i2 in indexesfwdback:
                    if i2 == rev_comp_dict[i1] and avg_qscore(i1qual) >= 30 and avg_qscore(i2qual) >= 30:                             #So, if the second index is the reverse complement of the first index,
